...
int quickSort(int arr[10000], int left, int right) {
    ...
    int pivot;
    if((arr[left]>arr[mid] && arr[left]<arr[right -1]) || (arr[left]<arr[mid] && arr[left]>arr[right -1]))
    {
        pivot = arr[left];
    }
    else if((arr[mid]>arr[left] && arr[mid]<arr[right -1]) || (arr[mid]<arr[left] && arr[mid]>arr[right -1]))
    {
        pivot = arr[mid];
        arr[mid] = arr[left];
        arr[left] = pivot;
